...
@pytest.fixture
def patient_dict_with_regex_filename() -> Dict:
    return {
        'hospitalId': 'hospa',
        'encounterId': 'ENC1111',
        'patientId': 'MRN1234',
...
@pytest.mark.parametrize(
    "input_file_name,input_file_count,mapping_file_name",
    [
        ("2022-02-18-patient-fwf.dat", "00001", "data-contract-fixed-width-regex.json"),
        ("2022-02-18-patient-fwf.dat", "00001", "data-contract-fixed-width-external-config.json")
    ],
)
def test_transform_patient_with_regex(
    data_contract_directory: str,
    monkeypatch,
    csv_directory: str,
    input_file_name: str,
    input_file_count: str,
    mapping_file_name: str,
    patient_dict_with_regex_filename: Dict,
    resource_meta_object: Meta,
):
    """
    Tests the transform function with a patient record and parametrized file names.
    ...
    # Marking to research
    diff = DeepDiff(
        patient_dict_with_regex_filename,
        actual_dict,
        ignore_order=True,
        truncate_datetime="hour",
        exclude_paths={"root['filePath']"}
